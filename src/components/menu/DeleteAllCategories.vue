<template>
  ...
  <div class="h-screen flex items-center justify-center">
    ...
    >
      ...
      <h5 class="mb-2 text-sm tracking-tight text-gray-900 dark:text-white">
        When you delete all categories , thier related menu items will be
        deleted as well
      </h5>
      ...
      <a
        href=""
        @click.prevent="deleteAllCategories"
...
export default {
  ...
  async mounted() {
    ...
    if (!user) {
      ...
      let res = await axios.get(
        `http://localhost:3000/items?locationId=${this.locationId}`
      );
  ...
  methods: {
    ...
    async getCategoryName() {
      let res = await axios.get(
        `http://localhost:3000/categories?id=${this.catId}&locationId=${this.locationId}&userId=${this.userId}`
      );
      if (res.status == 200) {
    ...
    async deleteAllCategories() {
      ...
      if (!allcateresault.includes(false) && !allitemresault.includes(false)) {
        this.$router.push({
          name: "Home",
        });
      } else {
